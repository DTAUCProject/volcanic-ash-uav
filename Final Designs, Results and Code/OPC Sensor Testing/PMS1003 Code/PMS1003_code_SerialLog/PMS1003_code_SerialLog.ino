//****************************************************************************************************************
//*Abstract: Code used to log total particle concentration of particles with assumed DENSITY and MAX_PARTICLE_SIZE
...
#define DENSITY 1.31e-6       // Bulk density of particulates in ug/um^3, PMS1003 assumes approx 3.67e-6 for PM readings
...
void loop()
{
  ...
  if(Serial.find(0x42)){
    ...
    if(buf[0] == 0x4d){
      if(checkValue(buf, LENG)){
        ...
        // Sum to get total concentration in ug/m^3, PM005 may wish to be commented out due to occasionally 
        // incorrect readings
  ...
  if (totalConc != totalConc_prev)  // If the sensor has updated the data
  {
    // GoBetwino Logging, uncomment to use GoBetwino (comment out PLX-DAQ code)
//    Serial.print("#S|PMSLOG|[");
//    Serial.print(float(millis())/1000, DP);
//    Serial.print(",");
